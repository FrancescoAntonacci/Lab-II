...
# File paths
filepath = 'dataFFT12/'
filename=['dataFFT01uFnoisy','dataFFT022uFnoisy','dataFFT047uFnoisy']
...
for idx, i in enumerate(filename):
    ...
    axes[idx, 0].plot(TT, vv, 'r--', label='T del best fit')
    axes[idx, 0].grid()
    axes[idx, 0].legend()
    
    # Plot frequency-domain (FFT)
    axes[idx, 1].plot(ff, v_tilde)
    axes[idx, 1].plot(xx, yy, 'r--',label='f0 del best fit')
...
plt.savefig(filepath_images + 'FFTRLCnoisy.png')
plt.show()
