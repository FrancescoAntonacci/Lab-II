...
##################
def ff(x, omega=2 * np.pi, delta=0.3, sigma=0,  a=3315,iter=10000):
    iter += 1
    f = 0
    x=x+sigma
...
t1, v1 = np.loadtxt(filepath + "data/191104/data6.txt", unpack=True)
t1 = t1
v1 = v1
...
p0=(2*np.pi/6.4e3,0.145,-3870)

"""plt.figure()
plt.plot(t1,v1)
plt.plot(xx,ff(xx,*p0))
plt.show()"""
...
plt.tight_layout()
plt.savefig(filepath + "bestfit_train.png")
plt.show()
