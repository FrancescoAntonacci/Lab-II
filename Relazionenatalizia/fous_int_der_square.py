...
def ff(x, a=1,omega=2 * np.pi,  iter=1000):
    iter += 1
    f = 0
...
def fi(x, a=1,omega=2 * np.pi, ft=1e-3, iter=1000):
    iter += 1
    f = 0
    for k in range(1, iter, 2):
...
def fd(x, a=1,omega=2 * np.pi, ft=1e-3, iter=1000):
    ...
    f = 0
    for k in range(1, iter, 2):
        f += G_hpf(k*omega,ft)*(2 / (k * np.pi)) * np.sin(k * x * omega + dphi_hpf(k*omega,ft))
    return a*f
...
plt.savefig(filepath+"integ_der_square.png")
plt.show()
