...
xx=numpy.linspace(min(x),max(x),50000)
...
pylab.plot(xx, ff(xx, pars[0], 0, 0), color='orange', label='ordine zero')
pylab.plot(xx,ff(xx,*pars), color='red', label="senza caduta")
pylab.plot(xx, ff(xx, pars[0]+200e-3, pars[1], pars[2]), color='purple', label="con caduta")
pylab.legend()
