...
pylab.xlabel(r'$V_dig$ [digit]',fontsize=18)
...
# call the routine
pars,covm=curve_fit(line,V_d,V_v,init,sigma,absolute_sigma=False) # <<<< NOTE THE absolute_sigma option
...
kappa2 = (w*(V_v-line(V_d,*pars))**2).sum()
...
for i in range(100):
    sigma=np.sqrt((pars[0]*sigma_V_d)**2+(sigma_V_v)**2)
    pars,covm=curve_fit(line,V_d,V_v,init,sigma,absolute_sigma=False)

pars1=pylab.copy(pars)
covm1=pylab.copy(covm)

w=1/sigma**2
kappa2 = (w*(V_v-line(V_d,*pars1))**2).sum()


corr1= pylab.copy(covm1)
for i in range(len(covm1)):
    for j in range(len(covm1)):
        corr1[i, j] = covm1[i,j]/(pylab.sqrt(covm1[i, i] * covm1[j, j]))
...
pylab.plot(xx,line(xx,*pars), color='red',label="Fit")
pylab.plot(xx,line(xx,*pars1),color='black',label="Fit con errori efficaci")
